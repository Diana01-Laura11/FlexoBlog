...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h1 class="text-center">Promociones Eliminadas</h1>
        <!-- DataTable with  -->
        <div class="card">
...
@endif
                    ...
                    <thead> 
                        <tr>
                            ...
                            <th>Fecha Inicio</th>
                            <th>Fecha Termino</th>
                            <th>Restaurar Registro</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($promotionDelete as $deletePromotion)
                            <tr>
                                <td>{{ $deletePromotion->promotion_id }}</td>
                                <td>{{ $deletePromotion->title }}</td>
                                <td>{{ $deletePromotion->subtitle }}</td>
                                <td>{{ $deletePromotion->start_date }}</td>
                                <td>{{ $deletePromotion->end_date }}</td>
                                {{-- <td>
                                    <!-- Botón para restaurar  -->
                                    <a href="{{ route('news.restoreNews', $deletePromotion->news_id) }}" class="btn btn-success btn-sm">Restaurar</a>
                                </td> --}}
                                <td>
                                    <form action="{{ route('promotions.restorePromotion', $deletePromotion->promotion_id) }}" method="POST">
                                        @csrf
