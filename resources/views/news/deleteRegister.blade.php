...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h1 class="text-center">Noticias Eliminadas</h1>
        <!-- DataTable with  -->
        <div class="card">
...
@endif
                    ...
                    <tbody>
                        @forelse ($newsDelete as $deleteNews)
                            <tr>
                                <td>{{ $deleteNews->news_id }}</td>
                                <td>{{ $deleteNews->title }}</td>
                                <td>{{ $deleteNews->creation_date }}</td>
                                <td>{{ $deleteNews->modification_date }}</td>
                                {{-- <td>
                                    <!-- Botón para restaurar  -->
                                    <a href="{{ route('news.restoreNews', $deleteNews->news_id) }}" class="btn btn-success btn-sm">Restaurar</a>
                                </td> --}}
                                <td>
                                    <form action="{{ route('news.restoreNews', $deleteNews->news_id) }}" method="POST">
                                        @csrf
