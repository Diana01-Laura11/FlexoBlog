...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h1 class="text-center">Categorias Eliminadas</h1>
        <!-- DataTable with  -->
        <div class="card">
            <div class="card-datatable table-responsive pt-0">
                ...
                <table id="deletedRegisterTable" class="table">
                    ...
                    <tbody>
                        @forelse ($categoryDelete as $deleteCategory)
                            <tr>
                                <td>{{ $deleteCategory->id }}</td>
                                <td>{{ $deleteCategory->name_category }}</td>
                                <td>{{ $deleteCategory->alias }}</td>
                                {{-- <td>
                                    <!-- Botón para restaurar  -->
                                    <a href="{{ route('news.restoreNews', $deleteCategory->news_id) }}" class="btn btn-success btn-sm">Restaurar</a>
                                </td> --}}
                                <td>
                                    <form action="{{ route('categories.restoreCategory', $deleteCategory->id) }}"
                                        method="POST">
                                        @csrf
