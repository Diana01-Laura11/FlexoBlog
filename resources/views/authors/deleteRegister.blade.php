...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h1 class="text-center">Autores Eliminados</h1>
        <!-- DataTable with  -->
        <div class="card">
...
@endif
                    ...
                    <tbody>
                        @forelse ($authorDelete as $deleteAuthor)
                            <tr>
                                <td>{{ $deleteAuthor->id_author }}</td>
                                <td>{{ $deleteAuthor->first_name }}</td>
                                <td>{{ $deleteAuthor->last_name }}</td>
                                <td>{{ $deleteAuthor->middle_name }}</td>
                                {{-- <td>
                                    <!-- Botón para restaurar  -->
                                    <a href="{{ route('news.restoreNews', $deleteAuthor->news_id) }}" class="btn btn-success btn-sm">Restaurar</a>
                                </td> --}}
                                <td>
                                    <form action="{{ route('authors.restoreAuthor', $deleteAuthor->id_author) }}" method="POST">
                                        @csrf
