...
document.getElementById('AddImagen').onclick = function () {
    ...
    // Actualiza el campo oculto con la URL correspondiente ene  este caso dependiendo si es para  img principal, secundaria, mini, banners etc.
    const selectedButton = document.getElementsByClassName("selectedButton")[0];
    if (selectedButton.id === 'Btnprincipal_Image') {
        console.log('Imagen seleccionada para Princial:', urlValue);  // Verifica si el valor se está asignando
        document.getElementById('hiddenPrincipal_Image').value = urlValue;  // Imagen principal
    } else if (selectedButton.id === 'Btnsecondary_Image') {
        console.log('Imagen seleccionada para secundaria:', urlValue);  // Verifica si el valor se está asignando
        document.getElementById('hiddenSecondaryImageUrl').value = urlValue;  // Imagen secundaria
    } else if (selectedButton.id === 'Btnmini_Image') {
        console.log('Imagen seleccionada para miniatura:', urlValue);  // Verifica si el valor se está asignando
        document.getElementById('hiddenmini_ImageImageUrl').value = urlValue;  // Imagen miniatura
    } else if (selectedButton.id === 'Btnbanners') {
        console.log('Imagen seleccionada para banners:', urlValue);  // Verifica si el valor se está asignando
        document.getElementById('hiddenBannersImageUrl').value = urlValue;  // Imagen Banners
    }

     // Actualiza el campo oculto con la URL para el envío al backend
    //  document.getElementById('hiddenImageUrl').value = urlValue;  // Asigna la URL al campo oculto
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            array_response.forEach((element) => {
                ...
                // Asegúrate de que la ruta comience desde "/assets"
                const startIndex = url.indexOf("assets");
                if (startIndex !== -1) {
                    var urlRelative = "/" + url.substring(startIndex); // Toma la parte después de "assets" con el prefijo "/"
                    
                    // Agrega el botón al listado de imágenes
                    listimages.innerHTML += "<button type='button' class='urlImg btn d-flex flex-column align-items-center' data-url='" + urlRelative + "'>" +
                        "<img src='" + urlRelative + "' alt='" + element.name + "' class='style-img'>" +
                        "<span class='mt-2'>" + element.name + "</span>" +
                    "</button>";
